...
int main(int argc, char* argv[])
{
    ...
    Network model;
    model.add_layer(new Conv2D("conv1", 10, 5, 1));
    model.add_layer(new Pooling("pool", 2, 0, 1, CUDNN_POOLING_MAX));
    model.add_layer(new Activation("relu", CUDNN_ACTIVATION_RELU)); 
    
    model.add_layer(new Conv2D("conv2", 20, 5, 1));
    model.add_layer(new Dropout("dropout", 0.2));
    model.add_layer(new Pooling("pool", 2, 0, 1, CUDNN_POOLING_MAX));
    model.add_layer(new Activation("relu", CUDNN_ACTIVATION_RELU));

    model.add_layer(new Linear("linear1", 320));
    model.add_layer(new Activation("relu", CUDNN_ACTIVATION_RELU));
    
    model.add_layer(new Linear("linear2", 50));
    model.add_layer(new Activation("relu", CUDNN_ACTIVATION_RELU));

    model.add_layer(new Linear("linear3", 10));
